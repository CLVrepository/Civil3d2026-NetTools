    ...
    public static class UflsPlaceAccessManholeCommands
    {
        ...
        public static void PlaceAccessManhole()
        {
            ...
            try
            {
                ...
                // ------------------------------------------------------------
                // 1. Select the Type 2 box. The box supplies the network,
                //    Parts List, sump elevation, and source name only.
                // ------------------------------------------------------------
                ...
                // ------------------------------------------------------------
                // 2. User chooses barrel, lid, and rim elevation source.
                //    Type I / Type IA is deliberately NOT a user choice.
                // ------------------------------------------------------------
                ...
                // ------------------------------------------------------------
                // 3. Select the UFLS_MH_MARK. It is the actual access-manhole
                //    center; the Type 2 box location is intentionally ignored.
                // ------------------------------------------------------------
                ObjectId markId;
                using (doc.LockDocument())
                using (Transaction tr = db.TransactionManager.StartTransaction())
                ...
                // ------------------------------------------------------------
                // 4. Resolve the exact access-structure family and barrel size
                //    from the selected network Parts List, then create the
                //    circular structure with no pipes.
                // ------------------------------------------------------------
                using (doc.LockDocument())
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    // Establish the elevation controls BEFORE setting the sump.
                    // This prevents Civil 3D from treating the sump as depth-driven.
            ...
            int lidInches)
        {
            ...
            // The current catalog should normally produce one exact match.
            // If it exposes multiple descriptions for the same physical size,
            // use the first catalog entry rather than asking the user to choose
            // a catalog implementation detail.
