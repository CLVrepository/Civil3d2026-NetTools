...
{
    /// <summary>
    /// Imports Prompt 3 roadway centerline / ROW / private street framework CSV files.
    ///
    /// Command names:
    ///   CLV_ROADS_IMPORT
    ///   CLV_3_ROADS_IMPORT
    ///   CLV-3-ROADS-IMPORT
    ///   SURVEY-ROADS-CSV-IMPORT
    ///   SURVEY-ROADS-CSV-IMPORT
    ///
    /// The primary importer expects the Prompt 3 road framework CSV fields.
    /// It also remains compatible with generic reference CSV rows that use Start/End coordinates, Bearing/Distance, and curve fields.
    /// Prompt 3 rows are drawn from the final boundary/control-anchored coordinates exported by the prompt.
    /// Bearing/distance rows use AdjustedDistance when present, otherwise Distance.
    /// </summary>
    public class RoadsCsvImportCommands
    {
        private const string RoadDefaultGeometryLayer = "V-MAPL-ROAD-CL";
        ...
        [CommandMethod("CLV_ROADS_IMPORT")]
        public void ClvRoadsImport()
        {
            RunRoadsImport();
        }

        [CommandMethod("CLV_3_ROADS_IMPORT")]
        public void Clv3RoadsImport()
        {
            RunRoadsImport();
        }

        [CommandMethod("CLV-3-ROADS-IMPORT")]
        public void Clv3RoadsDashImport()
        {
            RunRoadsImport();
        }

        [CommandMethod("SURVEY-ROADS-CSV-IMPORT")]
        public void SurveyRoadsCsvImport()
        {
            RunRoadsImport();
        }

        private static void RunRoadsImport()
        {
            ...
            {
                ed.WriteMessage("\nROADS IMPORT cancelled.");
                return;
            }

            List<RoadCsvRow> rows;
            try
            {
                rows = RoadCsvRow.Load(csvPath);
            }
            catch (System.Exception ex)
            {
                ed.WriteMessage($"\nUnable to read road framework CSV: {ex.Message}");
                return;
            }

            if (rows.Count == 0)
            {
                ed.WriteMessage("\nNo road framework rows were found in the selected CSV.");
                return;
            }

            RoadImportSummary summary = new RoadImportSummary(csvPath);
            Dictionary<int, ResolvedGeometry> closedTraverseOverrides = ResolveClosedOuterTraverseRows(rows, summary);
            ed.WriteMessage("\nROADS IMPORT defaults: Prompt 3 centerline-only CSV aware. Only ROAD_CENTERLINE / DERIVED_ROAD_CENTERLINE rows are imported. Road edges, ROW, ties, notes, and QA reference rows are skipped. Labels are limited to street names from RoadName.");

            using (doc.LockDocument())
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                EnsureDefaultLayers(db, tr);
                EnsureLayer(db, tr, RoadDefaultGeometryLayer, 2, "M");
                ...
                foreach (RoadCsvRow row in rows.OrderBy(r => r.Segment))
                {
                    if (!IsPrompt3CenterlineImportRow(row))
                    {
                        summary.AddSkipped(row.Segment, $"Skipped non-centerline Prompt 3 row. FeatureType='{row.FeatureType}', ReferenceClass='{row.ReferenceClass}', Type='{row.Type}'.");
                        continue;
                    }
                    ...
                    string geometryLayerName = layerName;
        ...
        private static string? PromptForCsvPath()
        {
            ...
            {
                Title = "Select Prompt 3 Road Centerline / ROW Framework CSV",
        ...
        private static void EnsureDefaultLayers(Database db, Transaction tr)
        {
            ...
            EnsureLayer(db, tr, "V-MAPL-ROAD-CE", 40, "M");
        ...
        private static string ResolveLayer(RoadCsvRow row, bool warning)
        {
            // The current Prompt 3 workflow is centerline-only.  Even if an older
            // CSV includes edge/ROW layer names, this importer only creates
            // centerlines on the standard road-centerline layer, or QA centerlines
            // on the QA layer.
            ...
            return RoadDefaultGeometryLayer;
        }

        private static bool IsPrompt3CenterlineImportRow(RoadCsvRow row)
        {
            string featureType = row.FeatureType.Trim().ToUpperInvariant();
            string referenceClass = row.ReferenceClass.Trim().ToUpperInvariant();
            string type = row.Type.Trim().ToUpperInvariant();

            if (type == "NOTE" || type == "POINT_TIE" || type == "RADIAL")
            {
                return false;
            }

            return featureType == "ROAD_CENTERLINE" ||
                   referenceClass == "ROAD_CENTERLINE" ||
                   referenceClass == "DERIVED_ROAD_CENTERLINE";
        }

        private static short DefaultColorForLayer(string layerName, RoadCsvRow row)
        {
            ...
            if (layerName.Contains("ROAD-CE", StringComparison.OrdinalIgnoreCase)) return 40;
        ...
        private static bool TryResolveStartEnd(RoadCsvRow row, Dictionary<int, ResolvedGeometry> closedTraverseOverrides, out Point3d start, out Point3d end, out string note)
        {
            note = string.Empty;
            ...
            // Prompt 3 road framework geometry is already tied to the final boundary/control coordinate system by the CSV.
            ...
            note = "Missing usable Start/End coordinates or Start + Bearing/Distance. Prompt 3 road framework CSV should be tied to the final boundary/control coordinate system.";
            return false;
        }


        private static Dictionary<int, ResolvedGeometry> ResolveClosedOuterTraverseRows(List<RoadCsvRow> rows, RoadImportSummary summary)
        {
            Dictionary<int, ResolvedGeometry> result = new Dictionary<int, ResolvedGeometry>();
            List<RoadCsvRow> candidates = rows
            ...
            List<RoadCsvRow> chain = SelectLongestSequentialChain(candidates);
            if (chain.Count < 3)
            {
        ...
        private static bool IsClosedOuterTraverseCandidate(RoadCsvRow row)
        {
            if (row.ImportStatus.Equals("DO_NOT_IMPORT", StringComparison.OrdinalIgnoreCase) ||
        ...
        private static List<RoadCsvRow> SelectLongestSequentialChain(List<RoadCsvRow> candidates)
        {
            List<RoadCsvRow> best = new List<RoadCsvRow>();
            List<RoadCsvRow> current = new List<RoadCsvRow>();
            int? prior = null;

            foreach (RoadCsvRow row in candidates.OrderBy(r => r.Segment))
            {
        ...
        private static bool TryResolveAnchorStart(List<RoadCsvRow> chain, out Point3d anchorStart, out string note)
        {
            RoadCsvRow first = chain[0];
        ...
        private static void BuildClosedTraverseOverride(List<RoadCsvRow> chain, double[] adjustedDistances, Vector2d[] directions, Point3d startPoint, Dictionary<int, ResolvedGeometry> result, double initialClosure, double finalClosure, double perimeter, string method, string anchorNote, double[]? originalDistances = null)
        {
        ...
        private static string NormalizeGeometryBuildMode(RoadCsvRow row)
        {
        ...
        private static bool ShouldUseBearingDistance(RoadCsvRow row, string buildMode)
        {
            ...
            if (buildMode.Equals("COORDINATE", StringComparison.OrdinalIgnoreCase) ||
        ...
        private static double? GetBuildDistance(RoadCsvRow row)
        {
            // Prompt 3 roadway CSVs may preserve both original map distance and
        ...
        private static string GetDistanceQaText(RoadCsvRow row)
        {
        ...
        private static void CheckCoordinateVsBearingDistance(RoadCsvRow row, Point3d start, Point3d end, ref string note)
        {
            double? buildDistance = GetBuildDistance(row);
        ...
        private static void AddCoordinateBearingConflictNote(RoadCsvRow row, Point3d start, Point3d csvEnd, Point3d calculatedEnd, ref string note, string action)
        {
        ...
        private static bool TryGetAnyPoint(RoadCsvRow row, out Point3d point)
        {
        ...
        private static void CheckWidthCallout(RoadCsvRow row, Point3d start, Point3d end, RoadImportSummary summary)
        {
        ...
        private static void AddRowWarnings(RoadImportSummary summary, RoadCsvRow row)
        {
        ...
        private static bool IsWarningRow(RoadCsvRow row)
        {
        ...
        private static string GetRowWarningText(RoadCsvRow row)
        {
        ...
        private static void AddReferenceLabel(BlockTableRecord modelSpace, Transaction tr, Point3d position, RoadCsvRow row, double textHeight, bool warning, string? prefix, string? layerName)
        {
            // ROADS IMPORT intentionally keeps labels minimal. The imported
            // road linework is for geometric review, and users should add CAD line labels
            // when they want to verify bearings/distances. Only street names from
            // RoadName are placed automatically.
            string text = row.RoadName.Trim();
            if (string.IsNullOrWhiteSpace(text))
            {
                return;
            }
        ...
        private static void ZoomToExtents(Editor ed, Extents3d extents)
        {
            ...
            {
                ed.WriteMessage("\nRoad framework CSV imported, but zoom-to-extents failed.");
            }
        }

        private static string? TryWriteReport(RoadImportSummary summary)
        {
            try
            {
                ...
                string reportPath = Path.Combine(directory, baseName + "_roads_import_report.txt");

                StringBuilder sb = new StringBuilder();
                sb.AppendLine("CLV Prompt 3 Road Centerlines-Only CSV Import Report");
        ...
        private static void WriteSummaryToCommandLine(Editor ed, RoadImportSummary summary, string? reportPath)
        {
            ed.WriteMessage("\nCLV ROADS IMPORT Complete (Centerlines Only)");
            ed.WriteMessage($"\n  CSV: {summary.CsvPath}");
        ...
        private sealed class RoadImportSummary
        {
            public RoadImportSummary(string csvPath)
            {
                CsvPath = csvPath;
            }
        ...
        private sealed class RoadCsvRow
        {
            ...
            public string WidthOffsetStatus { get; set; } = string.Empty;
            public string CenterlineMethod { get; set; } = string.Empty;
            public string LeftEdgeSegment { get; set; } = string.Empty;
            public string RightEdgeSegment { get; set; } = string.Empty;
            public string RelatedPrompt1Segment { get; set; } = string.Empty;
            public string RelatedPrompt2Segment { get; set; } = string.Empty;
            ...
            public bool HasAnyWidthData => RowWidth.HasValue || FullRowWidth.HasValue || HalfRowWidth.HasValue ||
                                           ...
                                           !string.IsNullOrWhiteSpace(WidthCallout) || !string.IsNullOrWhiteSpace(WidthCheckStatus) || !string.IsNullOrWhiteSpace(WidthOffsetStatus);

            public static List<RoadCsvRow> Load(string path)
            {
                string[] lines = File.ReadAllLines(path);
                if (lines.Length == 0)
                {
                    return new List<RoadCsvRow>();
                }
                ...
                List<RoadCsvRow> rows = new List<RoadCsvRow>();
                for (int i = 1; i < lines.Length; i++)
                {
                    ...
                    RoadCsvRow row = new RoadCsvRow
                    {
                        ...
                        WidthCallout = CoalesceString(map, values, "WidthCallout", "WidthSource"),
                        MeasuredWidth = CoalesceNullableDouble(map, values, "MeasuredWidth", "CADMeasuredWidthOrOffset"),
                        WidthError = CoalesceNullableDouble(map, values, "WidthError", "WidthOffsetError"),
                        WidthCheckStatus = CoalesceString(map, values, "WidthCheckStatus", "WidthOffsetStatus"),
                        WidthOffsetStatus = GetString(map, values, "WidthOffsetStatus"),
                        CenterlineMethod = GetString(map, values, "CenterlineMethod"),
                        LeftEdgeSegment = GetString(map, values, "LeftEdgeSegment"),
                        RightEdgeSegment = GetString(map, values, "RightEdgeSegment"),
                        RelatedPrompt1Segment = GetString(map, values, "RelatedPrompt1Segment"),
                        RelatedPrompt2Segment = GetString(map, values, "RelatedPrompt2Segment"),
                        OffsetDistance = CoalesceNullableDouble(map, values, "OffsetDistance", "MapWidthOrOffset"),
            ...
            private static string CoalesceString(Dictionary<string, int> map, List<string> values, params string[] keys)
            {
            ...
            private static double? CoalesceNullableDouble(Dictionary<string, int> map, List<string> values, params string[] keys)
            {
                foreach (string key in keys)
                {
